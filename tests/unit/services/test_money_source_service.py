...
def _service(mock_money_source_repo, mock_person_repo, mock_movement_repo):
    return MoneySourceService(mock_money_source_repo, mock_person_repo, mock_movement_repo)


def test_create_lanza_404_si_persona_no_existe(
    mock_money_source_repo, mock_person_repo, mock_movement_repo
):
    mock_person_repo.get_by_id.return_value = None
    service = _service(mock_money_source_repo, mock_person_repo, mock_movement_repo)

    with pytest.raises(HTTPException) as exc:
        service.create_money_source(MoneySourceCreate(person_id=99, name="Nequi"))
...
def test_create_lanza_409_si_nombre_duplicado(
    ...
    with pytest.raises(HTTPException) as exc:
        service.create_money_source(MoneySourceCreate(person_id=1, name="Nequi"))
...
def test_create_con_balance_inicial_crea_movimiento_adjustment(
    ...
    mock_money_source_repo.create.return_value = {
        "id": 5, "person_id": 1, "name": "Nequi",
        "name_normalized": "nequi", "balance": 100, "enabled": 1, "created_at": "x",
    }
    service = _service(mock_money_source_repo, mock_person_repo, mock_movement_repo)

    service.create_money_source(MoneySourceCreate(person_id=1, name="Nequi", balance=100))
...
def test_deposit_actualiza_balance_y_crea_movimiento(
    ...
    service.deposit(1, DepositRequest(amount=50))
...
def test_delete_lanza_400_si_tiene_movimientos(
    ...
    mock_money_source_repo.get_by_id.return_value = {"id": 1, "balance": 0, "enabled": 1}
    mock_movement_repo.has_movements.return_value = True
    service = _service(mock_money_source_repo, mock_person_repo, mock_movement_repo)

    with pytest.raises(HTTPException) as exc:
        service.delete_money_source(1)
