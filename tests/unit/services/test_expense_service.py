"""Pruebas unitarias de ExpenseService con mocks de los 5 repositorios."""
...
def _service(
    mock_expense_repo, mock_person_repo, mock_money_source_repo,
    mock_movement_repo, mock_budget_repo,
):
    # check_budgets dentro del service llamará a budget_repo.get_enabled_by_person
    # y a expense_repo.get_spent_in_period; los hacemos no-op por defecto.
    mock_budget_repo.get_enabled_by_person.return_value = []
...
def test_create_lanza_404_si_persona_no_existe(
    ...
    data = ExpenseCreate(
        person_id=99, amount=1000, description="X", date="2026-05-09"
    )
...
def test_create_con_money_source_decrementa_balance(
    ...
    data = ExpenseCreate(
        person_id=1, amount=200, description="Almuerzo",
...
def test_get_summary_delega_a_repo(
    ...
    result = service.get_summary(person_id=1)
